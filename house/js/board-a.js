  ...
  var LABEL = { agentic: "Marlowe", individual: "Individual", auto_grok: "Auto-Grok", joint: "Joint", combined: "House" };
  ...
  var JOBS = [
    { t: "06:30", days: [1, 2, 3, 4, 5], name: "Policy Pack", who: "Marlowe", role: "Full pack. World + WH. No trading." },
    { t: "09:30", days: [1], name: "Desk UX", who: "Marlowe", role: "Weekly phone-desk pass." },
    { t: "09:50", days: [1, 2, 3, 4, 5], name: "Autopilot AM", who: "Marlowe", role: "Name pick. Emails Grok to place." },
    { t: "10:00", days: [1], name: "Scorecard", who: "Marlowe", role: "Weekly process KEEP/CHANGE." },
    { t: "10:00", days: [1, 2, 3, 4, 5], name: "Snapshot", who: "Grok", role: "Book dump + mechanical stops." },
    { t: "11:50", days: [1, 2, 3, 4, 5], name: "Eyes", who: "Marlowe", role: "Risk-first pick. Quiet if \u226580% in." },
    { t: "13:50", days: [1, 2, 3, 4, 5], name: "Eyes", who: "Marlowe", role: "Risk-first pick. Quiet if \u226580% in." },
    { t: "14:50", days: [1, 2, 3, 4, 5], name: "Autopilot PM", who: "Marlowe", role: "Last same-day pick." },
    ...
    { t: "on SELL", days: [], name: "Process review", who: "Marlowe", role: "KEEP/CHANGE on the next pick." }
  ];
  var CAL = [
  ...
  function jobWhen(j) {
    ...
    if (!j.days || !j.days.length) return j.t;
    if (j.days.length === 1) return names[j.days[0]] + " " + j.t;
    return j.t;
  }
  ...
  function nextJob(now) {
    ...
    for (var add = 0; add < 8; add++) {
      ...
      for (var i = 0; i < JOBS.length; i++) {
        var j = JOBS[i];
        if (!j.days || j.days.indexOf(dow) < 0) continue;
        if (!/^\d{2}:\d{2}$/.test(j.t)) continue;
        var parts = j.t.split(":");
        var when = new Date(day);
        when.setHours(Number(parts[0]), Number(parts[1]), 0, 0);
        if (when <= now) continue;
        if (!best || when < best.when) best = { when: when, t: jobWhen(j), name: j.name, role: j.role, who: j.who, add: add };
      }
      if (best) break;
    }
    return best;
